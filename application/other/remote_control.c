/**
  ...
  * @cond       总结执行流（关键点）：
                收到串口/DMA 数据 → 在 USART3_IRQHandler 中检测到完整帧 → 调用 sbus_to_rc 或协议专用函数 → 
                在这些函数内部直接对传入的 RC_ctrl_t *（即 &rc_ctrl）做字段赋值，从而更新全局 rc_ctrl。
                若通信模块接收到来自其他板的 RC 数据，会用 memcpy 将远端数据覆盖到本地的 rc_ctrl。
                若数据被判定异常，会在 RC_data_is_error() 内清零。
  * @history
