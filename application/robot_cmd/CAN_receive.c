/**
  ...
  *  V2.4.0     Mar-06-2026     Richard         1. 添加了测试模块
  *                                             2. 添加了错误处理模块
  *
  @verbatim
  ==============================================================================
...
// 在.c文件中定义变量，在.h文件中声明变量：
// 只有一份变量实例（节省内存）
// 所有文件共享同一份数据（数据一致性）
// 符合 C 语言的最佳实践
uint8_t RX_DATE = 0;
// CAN 总线状态监测
static uint32_t can1_last_rx_time = 0;
static uint32_t can2_last_rx_time = 0;
static uint8_t can1_status = 0;
static uint8_t can2_status = 0;
static uint32_t can1_error_count = 0;
static uint32_t can2_error_count = 0;
...
void HAL_CAN_RxFifo0MsgPendingCallback(hcan_t * hcan)
{
    ...
    // 【调试】记录CAN2收到的所有StdId到全局变量
    static uint32_t can2_rx_id_buffer[10] = {0};  // 记录最近10个ID
    static uint8_t can2_rx_id_index = 0;
    if (hcan == &hcan2 && rx_header.IDE == CAN_ID_STD) {
        can2_rx_id_buffer[can2_rx_id_index] = rx_header.StdId;
        can2_rx_id_index = (can2_rx_id_index + 1) % 10;
    }
    (void)can2_rx_id_buffer;  // 防止编译器警告：调试用变量
    ...
    if(hcan->Instance == CAN1) {
        can1_last_rx_time = current_time;
        can1_status = 1;
    } else if(hcan->Instance == CAN2) {
        can2_last_rx_time = current_time;
        can2_status = 1;
    }
...
// clang-format off
#ifdef DEBUG
#define CAN_DEBUG_CHECK_STATUS(can_num) Check_CAN_Status(can_num)
#else
#define CAN_DEBUG_CHECK_STATUS(can_num) ((void)0)
#endif
// clang-format on

/** 
 *  @brief          检查 CAN 总线状态
 *  @param[in]      can_num: CAN 编号 (1 或 2)
 *  @retval         0: CAN 总线离线
 *                  1: CAN 总线正常
 *                  2: CAN 总线错误过多
 *  @note           测试用
 */
uint8_t Check_CAN_Status(uint8_t can_num)
{
    ...
    if(can_num == 1) {
        last_rx_time = can1_last_rx_time;
        status = can1_status;
        error_count = &can1_error_count;
    } else if(can_num == 2) {
        last_rx_time = can2_last_rx_time;
        status = can2_status;
        error_count = &can2_error_count;
    } else {
        return 0;
    }
    
    // 如果超过设定时间没有收到数据，判定为离线
    if(current_time - last_rx_time > CAN_OFFLINE_TIME) {
        return 0;  // 离线
    }
    ...
    return status ? 1 : 0;  // 正常
}

/**
 * @brief          获取 CAN 总线最后接收时间
 * @param[in]      can_num: CAN 编号 (1 或 2)
 * @retval         最后接收时间 (ms)
 */
uint32_t Get_CAN_LastRxTime(uint8_t can_num)
{
    if(can_num == 1) {
        return can1_last_rx_time;
    } else if(can_num == 2) {
        return can2_last_rx_time;
    }
    return 0;
}
