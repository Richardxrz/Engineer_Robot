...
#include "robot_param_mecannum.h"
...
#if (MECHANICAL_ARM_TYPE == MECHANICAL_ARM_ENGINEER_ARM)
        // 在HOLD模式下，底盘跟随机械臂J0
        if (GetMechanicalArmMode() == MECHANICAL_ARM_HOLD)
        {
            chassis.mode = CHASSIS_FOLLOW;
        }
        else
...
void ChassisReference(void)
{
    ...
    else if (chassis.mode == CHASSIS_FOLLOW)
    {
        ...
        {
            chassis.reference_rc.vx=fp32_deadline(chassis.rc->rc.ch[2],-CHASSIS_RC_DEADLINE,CHASSIS_RC_DEADLINE)/CHASSIS_RC_MAX_RANGE*CHASSIS_RC_MAX_SPEED;
            chassis.reference_rc.vy=fp32_deadline(-chassis.rc->rc.ch[3],-CHASSIS_RC_DEADLINE,CHASSIS_RC_DEADLINE)/CHASSIS_RC_MAX_RANGE*CHASSIS_RC_MAX_SPEED;
        }
        else
        {
...
#if (MECHANICAL_ARM_TYPE == MECHANICAL_ARM_ENGINEER_ARM)
        // 在HOLD模式下，底盘跟随机械臂J0角度
        if (GetMechanicalArmMode() == MECHANICAL_ARM_HOLD)
        {
            chassis.reference.wz = PID_calc(&chassis_pid.follow, 0, GetMechanicalArmJ0Angle());
        }
        else
        {
